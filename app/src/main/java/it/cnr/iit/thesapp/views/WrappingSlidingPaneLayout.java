...
public class WrappingSlidingPaneLayout extends ViewGroup {
	...
	private static final String TAG                   = "WrappingSlidingLayout";
	/**
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (int i = 0; i < childCount; i++) {
			...
			if (lp.height == LayoutParams.WRAP_CONTENT) {
				childHeightSpec = MeasureSpec.makeMeasureSpec(maxLayoutHeight, MeasureSpec
						.AT_MOST);
				...
				if (canSlide && child != mSlideableView) {
					if (lp.width < 0 && (measuredWidth > fixedPanelWidthLimit || lp.weight > 0)) {
						...
						if (skippedFirstPass) {
							...
							childHeightSpec = MeasureSpec.makeMeasureSpec(child
											.getMeasuredHeight(),
									MeasureSpec.EXACTLY);
						}
					...
					if (canSlide) {
						...
						final int childWidthSpec = MeasureSpec.makeMeasureSpec(newWidth,
								MeasureSpec.EXACTLY);
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int i = 0; i < childCount; i++) {
			...
			if (lp.slideable) {
				...
				final int range = Math.min(nextXStart, childWidth - paddingRight - mOverhangSize) -
								  xStart - margin;
				mSlideRange = range;
				lp.dimWhenOffset =
						xStart + (width - childWidth) + lp.leftMargin + range + childWidth / 2 >
						width - paddingRight;
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (!mCanSlide && action == MotionEvent.ACTION_DOWN && getChildCount() > 1) {
			...
			if (secondChild != null) {
				mPreservedOpenState = !mDragHelper.isViewUnder(secondChild, (int) ev.getX(),
						(int) ev.getY());
			}
		}
		...
		switch (action) {
			case MotionEvent.ACTION_DOWN: {
				...
				if (mDragHelper.isViewUnder(mSlideableView, (int) x, (int) y) && isDimmed(
						mSlideableView)) {
					interceptTap = true;
				}
				break;
		...
		switch (action & MotionEventCompat.ACTION_MASK) {
			...
			case MotionEvent.ACTION_UP: {
				if (isDimmed(mSlideableView)) {
					...
					final int slop = mDragHelper.getTouchSlop();
					if (dx * dx + dy * dy < slop * slop && mDragHelper.isViewUnder(mSlideableView,
							(int) x, (int) y)) {
	...
	private void onPanelDragged(int newLeft) {
		...
		final int leftBound =
				getMeasuredWidth() - mSlideableView.getMeasuredWidth() + getPaddingLeft() +
				lp.leftMargin;
	...
	boolean smoothSlideTo(float slideOffset, int velocity) {
		...
		Logs.ui("Sliding to " + slideOffset);
		final LayoutParams lp = (LayoutParams) mSlideableView.getLayoutParams();
	...
	private class DragHelperCallback extends ViewDragHelper.Callback {
		...
		public void onViewReleased(View releasedChild, float xvel, float yvel) {
			...
			int left = getMeasuredWidth() - releasedChild.getMeasuredWidth() + getPaddingLeft() +
					   lp.leftMargin;
